...
>
> 📋 **Auditoria total 2026-04-17:** varredura arquitetural completa em 20 lentes (187 sub-itens) disponível em `docs/audit-fine-tooth-comb-2026-04.md`. Plano de implementação em 11 sprints detalhado em `docs/implementation-plan.md`. Os itens abaixo continuam válidos; a auditoria adiciona uma camada de correções estruturais (RBAC granular, 4-eyes, audit hash-chain, schema hardening em `bigint` cents, particionamento, observability completa).
>
...
| ~~12~~ | ~~**Cloudflare WAF**~~ — ✅ **CONCLUÍDO (2026-04-17)** — Cloudflare Managed Free Ruleset ativo (26 regras, todas em modo block). Rate limit criado: 17 req/10 s por IP em `/api/` (≈100 req/min — máximo do plano Free). ⚠️ OWASP Core Ruleset requer plano Pro ($20/mês); considerar upgrade quando houver tráfego real. Zone: `clinipharma.com.br` · Ruleset ID: `d5e62209f1284719869be7d640da0da6` | `docs/roadmap-90pts.md` A2                                                                                                             |
| 11b    | **Supabase PITR** — ativar Point-in-Time Recovery para reduzir RPO de 24h → 5 min. **Adiar até ter clientes reais ativos.** Custo: $100/mês (7 dias) · $200 (14 dias) · $400 (28 dias). Recomendação: 7 dias quando houver movimentação financeira real. Ativar em: `Supabase Dashboard → Project → Settings → Add-ons → Point in Time Recovery`                                                      | `docs/disaster-recovery.md`                                                                                                            |
...
| #      | Pendência                                                                                                                                                                                                                                                                                                                                                                                                   | Onde documentado                                                                                             |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | --- |
| ~~14~~ | ~~**Zenvia SMS + WhatsApp**~~ — ✅ **SMS CONCLUÍDO (2026-04-17)** — conta criada, token configurado, sender `Clinipharma` ativo. `ZENVIA_API_TOKEN` e `ZENVIA_SMS_FROM` atualizados no Vercel (production + preview + development). **WhatsApp adiado:** conectar WABA da Zenvia com o WhatsApp Business existente quando a plataforma estiver em operação real. `ZENVIA_WHATSAPP_FROM` permanece pendente. | `docs/go-live-checklist.md` item 3                                                                           |
...
| ~~25~~ | ~~**Auditoria WCAG 2.1**~~          | ✅ **CONCLUÍDO (2026-04-18)** — 3 camadas (ESLint `jsx-a11y`, unit DOM com `axe-core`, E2E com `@axe-core/playwright`). 18 novos testes em `tests/unit/a11y/ui-components.test.tsx` (1905/1905 verde). 16 componentes corrigidos (dialog, sheet, pagination, sidebar, notification-bell, global-search, header, address-book, save-template-modal, categories-manager, ocr-analysis-button, registro, profile, registrations, catalog-filters). Relatório completo em `docs/compliance/a11y-audit-2026-04.md`. |
...
| Item   | Descrição                                                                                                                 |
| ------ | ------------------------------------------------------------------------------------------------------------------------- |
| ~~B1~~ | ~~**Emissão de NF-e**~~ — ✅ **v6.8.0**: NFS-e implementada via Nuvem Fiscal. Aguarda apenas credenciais + migration 042. |
