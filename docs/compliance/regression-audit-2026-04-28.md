...
- [x] **Pagamento — hot-incident 2026-04-29 #2: clínica fica sem UI para pagar quando farmácia aprova o pedido**
  - **Sintoma**: clínica recebe a notificação "documentação aprovada, aguardando pagamento" mas, ao abrir o pedido, vê apenas a mensagem "Aguardando a geração da cobrança pelo administrador". Não há onde clicar pra pagar — o pedido fica num limbo até um admin de plataforma aparecer e criar a cobrança manualmente.
  - **Causa-raiz**: três camadas de defeito empilhadas:
    1. **Geração não-automática**: `services/document-review.ts#transitionDocStatus` mudava o status para `AWAITING_PAYMENT` mas não disparava a criação da cobrança no Asaas. O `payments` insert na criação do pedido (em `services/orders.ts`) ficava com `payment_method='MANUAL'` e `asaas_payment_id=null` para sempre.
    2. **API gated em admin**: `app/api/payments/asaas/create/route.ts` exigia `requireRole(['SUPER_ADMIN', 'PLATFORM_ADMIN'])`, então mesmo que a clínica quisesse forçar a geração, recebia 401.
    3. **UI dead-end**: `components/orders/payment-options.tsx` mostrava o texto "Aguardando geração da cobrança pelo administrador" para qualquer não-admin. Bug latente bonus: a query do API route fazia `clinics(profiles(email, phone))` sem FK direta entre `clinics` e `profiles` — ou seja, mesmo se um admin tivesse clicado no botão, teria recebido `PGRST200 Could not find a relationship`. O fluxo nunca tinha funcionado.
  - **Fix #1 — `lib/payments/asaas-charge.ts` (novo helper `server-only`)**: extrai a lógica de criação de cobrança em uma função `generateAsaasChargeForOrder(orderId)` idempotente — se já existe `payments.asaas_payment_id` em `PENDING`, retorna sem chamar Asaas de novo. Resolve `clinic_members → profiles` separadamente (corrige o `PGRST200`). Trata erros estruturados do Asaas: quando o `400 invalid_object` "CPF/CNPJ informado é inválido" volta, o helper devolve `'CNPJ X é inválido segundo o Asaas. Atualize o CNPJ no perfil da clínica.'` em vez de uma mensagem genérica.
  - **Fix #2 — auto-trigger em `services/document-review.ts`**: quando docs são aprovados e o status vira `AWAITING_PAYMENT`, chama `generateAsaasChargeForOrder(orderId)` em best-effort (try/catch + log) ANTES de notificar a clínica. Se Asaas estiver fora, a transição de status acontece mesmo assim e a clínica pode retentar pelo botão da UI.
  - **Fix #3 — `app/api/payments/asaas/create/route.ts`**: troca `requireRole(['SUPER_ADMIN', 'PLATFORM_ADMIN'])` por uma checagem em duas etapas — admin de plataforma OU admin da clínica dona do pedido (verificado via `clinic_members`). Como o helper é idempotente, a clínica clicar no botão duas vezes não cria cobranças duplicadas no Asaas.
  - **Fix #4 — `components/orders/payment-options.tsx`**: substitui a mensagem dead-end por um botão "Gerar opções de pagamento" para clínicas. Botão chama o mesmo endpoint, recebe os dados de PIX/Boleto/Cartão, e a UI já existente passa a renderizar QR code, link de boleto e link de cartão.
  - **Fix #5 — bug latente em `components/orders/order-detail.tsx`**: a prop passada para `<PaymentOptions amount={...} />` lia `order.total_amount` (campo inexistente) em vez de `order.total_price`. Resultado: o componente exibia `R$ 0,00` ao usuário. Trocado para `total_price`.
  - **Backfill — `scripts/backfill-asaas-charge.ts`**: script standalone (não importa `server-only`) que aplica a mesma lógica em pedidos antigos travados em `AWAITING_PAYMENT` sem `asaas_payment_id`. Roda via `npx tsx scripts/backfill-asaas-charge.ts <orderId>` e é idempotente.
  - **Bloqueador externo descoberto durante o backfill**: a clínica seed `Clínica Exemplo` tem CNPJ `11.222.333/0001-44` (placeholder). O Asaas em produção (`api.asaas.com/v3`) rejeita esse CNPJ com `400 invalid_object`. O helper agora retorna a mensagem específica para o usuário; a clínica precisa atualizar para um CNPJ válido OU `ASAAS_API_URL` precisa ir pra sandbox em ambientes de teste.
  - **Verificações**: `npx tsc --noEmit` ✓, `npx vitest run` ✓ 1988/1988, `npx eslint` ✓ zero erros, `./scripts/claims/run-all.sh` ✓ 17/17.
